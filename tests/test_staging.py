...
def test_stage_source_file_falls_back_to_cp_after_retries(
    ...
    def _always_eio_chunked(
        *,
        ...
        del source_path, staged_path, source_size, console, show_progress
        raise OSError(errno.EIO, "Input/output error")

    def _fake_cp(source_path: Path, staged_path: Path) -> None:
        staged_path.write_bytes(source_path.read_bytes())

    monkeypatch.setattr("walkmarr.process._copy_file_chunked", _always_eio_chunked)
    monkeypatch.setattr("walkmarr.process._copy_file_with_cp", _fake_cp)
    monkeypatch.setattr("walkmarr.process.time.sleep", lambda _seconds: None)
